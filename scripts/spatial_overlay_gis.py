#!/usr/bin/env python3
"""
Spatial Overlay: AQI 測站 + 避難所疊圖 (GIS版本)
使用GeoPandas精確移除海上避難所
"""
...
class SpatialOverlayGIS:
    ...
    def create_taiwan_boundary(self):
        """創建台灣邊界多邊形"""
        # 台灣大致邊界坐標
        taiwan_coords = [
            [121.5, 25.3],  # 北端
    ...
    def load_and_filter_shelters(self):
        """載入並過濾避難所資料"""
        try:
            # 載入避難所資料
            ...
            print(f"GIS處理失敗: {e}")
            # 如果GIS處理失敗，使用簡單過濾
            return self.load_shelter_simple()
    
    def load_shelter_simple(self):
        """簡單的避難所載入方式（備用）"""
        try:
            df = pd.read_csv('data/shelters_cleaned.csv', encoding='utf-8-sig')
            ...
            # 簡單的邊界過濾
            filtered_df = df[
                (df['緯度'] >= 21.8) & (df['緯度'] <= 25.5) &
                (df['經度'] >= 119.5) & (df['經度'] <= 122.5)
            ]
            
            print(f"簡單過濾後避難所數量: {len(filtered_df)}")
            return filtered_df
    ...
    def create_map(self):
        """創建空間疊圖地圖"""
        print("正在創建GIS版Spatial Overlay地圖...")
        
        # 載入資料
        self.aqi_data = self.create_mock_aqi_data()
        self.shelter_data = self.load_and_filter_shelters()
        ...
        # 儲存地圖
        output_file = 'outputs/spatial_overlay_gis.html'
        m.save(output_file)
...
def main():
    """主程式"""
    print("=== Spatial Overlay: AQI 測站 + 避難所疊圖 (GIS版本) ===")
    
    # 確保輸出目錄存在
    os.makedirs('outputs', exist_ok=True)
    
    # 創建地圖
    map_creator = SpatialOverlayGIS()
    output_file, indoor_count, outdoor_count = map_creator.create_map()
    
    print(f"\nGIS版Spatial Overlay地圖創建完成！")
